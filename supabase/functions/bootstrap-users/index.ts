...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    let pwnedCount = 0;
    try {
      pwnedCount = await leakedPasswordCount(initialPassword);
    } catch (error) {
    ...
    if (pwnedCount > 0) {
      return json({ error: "Dieses Startpasswort ist aus bekannten Datenleaks bekannt. Bitte wähle ein anderes Passwort." }, 400);
    }
